...
import { ChevronRight, ListIcon, LayoutDashboard } from "lucide-react";
...
export function Sidebar() {
  ...
  // Fallback: when useMenu returns empty, render from resources (imported directly)
  const itemsToRender =
    menuItems.length > 0
      ? menuItems
      : resources
          .filter((r) => r.list && !r.meta?.hide)
          .map((r) => ({
            key: r.identifier ?? r.name,
            name: r.name,
            route: r.list,
    ...
    <ShadcnSidebar collapsible="icon" className={cn("border-none")}>
      ...
      >
        <SidebarNavLink
          ...
          isSelected={selectedKey === "/" || !selectedKey}
        />
        {itemsToRender.map((item: TreeMenuItem) => (
          <SidebarItem
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        {
          "bg-sidebar-primary text-sidebar-primary-foreground": isSelected,
        },
      )}
      asChild
    >
